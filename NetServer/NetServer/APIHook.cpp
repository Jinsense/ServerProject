...
CAPIHook CAPIHook::sm_LoadLibraryW  ((PSTR)"Kernel32.dll", (PSTR)"LoadLibraryW",
	(PROC) CAPIHook::LoadLibraryW);

CAPIHook CAPIHook::sm_LoadLibraryExA((PSTR)"Kernel32.dll", (PSTR)"LoadLibraryExA",
	(PROC) CAPIHook::LoadLibraryExA);

CAPIHook CAPIHook::sm_LoadLibraryExW((PSTR)"Kernel32.dll", (PSTR)"LoadLibraryExW",
	(PROC) CAPIHook::LoadLibraryExW);

CAPIHook CAPIHook::sm_GetProcAddress((PSTR)"Kernel32.dll", (PSTR)"GetProcAddress",
...
void CAPIHook::FixupNewlyLoadedModule(HMODULE hmod, DWORD dwFlags) 
{
		...
		) 
	{
		...
		for (CAPIHook* p = sm_pHead; p != NULL; p = p->m_pNext) 
		{
			...
			char szPathname[MAX_PATH];
			GetModuleFileName(NULL, (LPWSTR)szPathname, _countof(szPathname));
			char sz[1024];
			StringCchPrintf((LPWSTR)sz, _countof(sz), 
				L"[%4u - %s] impossible to find %S\r\n", 
				GetCurrentProcessId(), szPathname, p->m_pszCalleeModName);
			OutputDebugString((LPWSTR)sz);
#endif
			}
		}
	}
